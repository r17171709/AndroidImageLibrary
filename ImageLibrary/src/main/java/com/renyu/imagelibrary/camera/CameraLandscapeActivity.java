...
public class CameraLandscapeActivity extends BaseActivity implements CameraLandscapeFragment.TakenCompleteListener, CancelAdapt {
    ...
    public void permissionApply() {
        if (getSupportFragmentManager().getFragments().size() == 0) {
            CameraLandscapeFragment cameraFragment;
            if (getIntent().getSerializableExtra("cameraFunctions") != null) {
                cameraFragment = CameraLandscapeFragment.getInstance((ArrayList<CameraLandscapeFragment.CameraFunction>) getIntent().getSerializableExtra("cameraFunctions"));
            } else {
                cameraFragment = CameraLandscapeFragment.getInstance();
            }
